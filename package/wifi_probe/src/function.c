...
void *Sending_To_Server(void *p)
{
	...
	pthread_t tid = 0;
	int port = 80;
	//char ip[] = "144.123.43.119";     //山东数据
	//char ip[] = "183.60.200.186";     //数秒科技
	char ip[] = "123.56.226.128";       //北京睿柏
	memset(&servaddr, 0, sizeof(servaddr));
	servaddr.sin_family = AF_INET;									   //ipv4
	servaddr.sin_port = htons(port);								   //8000 port
	if (inet_pton(AF_INET, ip, &servaddr.sin_addr) <= 0) //ip
	{
		exit(1);
	}
	Init_thread_Share();
