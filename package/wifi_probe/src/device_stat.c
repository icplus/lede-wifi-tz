...
char *List_Element_To_Str(List *pList)
{
    ...
    Str_Length = sprintf(pBody, "{\"mid\":\"%x:%x:%x:%x:%x:%x\",\"datalist\":[", mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
    while (pCurrent)
    {
        Str_Length += sprintf(pBody + Str_Length, "{\"mac\":\"%02x:%02x:%02x:%02x:%02x:%02x\",\"rssi\":%d},",
                              pCurrent->Key.MAC_SLICE[0], pCurrent->Key.MAC_SLICE[1], pCurrent->Key.MAC_SLICE[2],
                              pCurrent->Key.MAC_SLICE[3], pCurrent->Key.MAC_SLICE[4], pCurrent->Key.MAC_SLICE[5],
    ...
    //const char *HttpHeader = "POST /demo/api/sjcjjk.act HTTP/1.1\r\nHost: www.sujustore.com\r\nContent-Type: text/plain\r\nAccept: */*\nAccept-Encoding: deflate, br\r\nConnection: close\r\nContent-Length:";
    //const char *HttpHeader = "POST /tdata/wifidata HTTP/1.1\r\nHost: dspapi.adpaas.cn\r\nContent-Type: text/plain\r\nAccept: */*\nAccept-Encoding: deflate, br\r\nConnection: close\r\nContent-Length:"; //
    const char *HttpHeader = "POST /onlypay_weiyou/TestSer HTTP/1.1\r\nHost: 123.56.226.128\r\nContent-Type: text/plain\r\nAccept: */*\nAccept-Encoding: deflate, br\r\nConnection: close\r\nContent-Length:"; //北京睿柏
